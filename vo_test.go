...
func TestViewField_Validate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			json:        `{}`,
			wantFound:   false,
			expectedErr: validator.ErrRequired,
		},
		{
		...
		},
		{
			...
			field:      Field[string]("password")(validator.MinLength(5)),
		...
		},
		{
			...
			field:       Field[string]("password")(validator.MinLength(6)),
			json:        `{"password": "short"}`,
			wantFound:   true,
			expectedErr: validator.ErrLengthMin,
		},
	}
